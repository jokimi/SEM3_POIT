#include "GA.h"
children GeneticAlgorithm::crossover(string parent1, string parent2) { //Скрещивание
...
string GeneticAlgorithm::mutatedGene(string gnome) { //Мутация
    while (true) {
...
string GeneticAlgorithm::createGnome() { //Создание геномов популяции
    string gnome = "0";
    while (true) {
...
int GeneticAlgorithm::calculatePath(string gnome) { // Подсчитывание маршрута коммивояжера
    int path = 0;
...
individual GeneticAlgorithm::findMinPath(vector<individual> population) { // Подсчитывание минимального маршрута коммивояжера
...
void GeneticAlgorithm::Solve() {
    int gen = 1; // Номер поколения
    vector<individual> population;
    individual temp;
    for (int i = 0; i < POP_SIZE; i++) { // Создание популяции
    ...
    cout << "\n";
    while (gen <= EVOL_NUM) { // Эволюции
        sort(population.begin(), population.end(), lessthan);
        vector<individual> new_population;
        
        for (int i = 0; i < POP_SIZE; i += 2) { // Скрещивание
            if ((i + 1) != POP_SIZE) {
        ...
        for (int i = 0; i < POP_SIZE; i++) { // Мутация
            individual p1 = population[i];
            while (true) {
...
int GeneticAlgorithm::randNum(int start, int end) { // Генерация рандомного числа в заданном диапазоне
    int r = end - start;
    int rnum = start + rand() % r;
    return rnum;
}
bool GeneticAlgorithm::repeat(string s, char ch) { // Проверка на повторение символов в строке
    for (unsigned int i = 0; i < s.size(); i++)
        if (s[i] == ch)
            return true;
    return false;
}
bool lessthan(struct individual t1, struct individual t2) { // Сравнение путей в разных геномах
